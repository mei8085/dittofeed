# Journey 全链路深度分析报告 (R5)

## 核心修正点

本报告是 R4 的修正版，重点修正了两个关键问题：

1. **segmentUpdate 触发条件** - 仅用户进入分段时发送（纠正"进入/退出"表述）
2. **reEvaluateSegmentsSignal 结论** - 只保留可证据结论，不做使用场景推断

---

## 1. 信号定义与 Handler 对应关系
...
### 2.1 reEvaluateSegmentsSignal: 可证据结论
...
**可证据结论**:
- ✅ 信号定义存在：`packages/backend-lib/src/journeys/userWorkflow.ts:58`
- ✅ Handler 存在：`packages/backend-lib/src/journeys/userWorkflow.ts:551`
- ❌ **未发现生产发送路径**

**搜索范围验证**:
- `packages/backend-lib/src/`: 仅 2 个匹配（定义和 Handler）
- `packages/api/src/`: 无匹配
- `packages/dashboard/src/`: 无匹配
...
**发送上下文（关键条件）**:
```typescript
// packages/backend-lib/src/journeys.ts:848-857
...
if (!segmentUpdate.currentlyInSegment) {
  return;  // 退出时直接返回，不发送信号
}
```

**前置检查** (journeys.ts:827-847):
```typescript
...
- 分段 ID 匹配
- **`currentlyInSegment === true`（仅进入分段时）**
...
const segmentUpdate: SegmentUpdate = {
  type: "segment",
  segmentId,
  currentlyInSegment: true,  // 硬编码为 true
...
对每个用户: signalWithStart + segmentUpdateSignal
        ↓
segmentUpdate.currentlyInSegment = true (硬编码)
```
...
**segmentUpdate 条件总结**:
- 两个发送入口均只在 `currentlyInSegment === true` 时发送
- **仅用户进入分段时发送**
...
| 信号 | 变量名 | 注册名 | 发送位置 | 发送方式 | 触发条件 |
|------|--------|--------|---------|---------|---------|
| **segmentUpdateSignal** | `segmentUpdateSignal` | `"segmentUpdate"` | `packages/backend-lib/src/journeys.ts:889` | `signalWithStart` | 用户进入分段时触发 SegmentEntry Journey (currentlyInSegment === true) |
| **segmentUpdateSignal** | `segmentUpdateSignal` | `"segmentUpdate"` | `packages/backend-lib/src/restartUserJourneyWorkflow/activities.ts:118` | `signalWithStart` | Journey 从 Paused 恢复为 Running 且 reEnter=true (currentlyInSegment === true 硬编码) |
...
| **reEvaluateSegmentsSignal** | `reEvaluateSegmentsSignal` | `"reEvaluateSegments"` | **未发现生产发送路径** | - | - |
...
| 信号 | 发送文件 | 行号 | 发送函数 | 触发条件 |
|------|---------|------|---------|---------|
| `segmentUpdateSignal` | journeys.ts | 889 | `signalWithStart` | 进入分段 (currentlyInSegment === true) |
| `segmentUpdateSignal` | restartUserJourneyWorkflow/activities.ts | 118 | `signalWithStart` | 恢复运行 (currentlyInSegment === true 硬编码) |
| `trackSignal` | journeys/userWorkflow/lifecycle.ts | 76 | `signalWithStart` | EventEntry 匹配事件 |
...
### 5.4 segmentUpdate 关键条件代码

| 发送入口 | 文件 | 行号 | 关键条件 |
|---------|------|------|---------|
| triggerSegmentEntryJourney | journeys.ts | 855-857 | `if (!segmentUpdate.currentlyInSegment) return;` |
| restartUserJourneysActivity | restartUserJourneyWorkflow/activities.ts | 93-98 | `currentlyInSegment: true` (硬编码) |

---

## 6. 修正总结

### 6.1 segmentUpdate 触发条件修正
...
- 代码证据:
  ...
  ```
  ```typescript
  // restartUserJourneyWorkflow/activities.ts:93-98
  const segmentUpdate: SegmentUpdate = {
    type: "segment",
    segmentId,
    currentlyInSegment: true,  // 硬编码为 true
...
### 6.2 reEvaluateSegmentsSignal 结论修正

**R4 表述**:
- "使用场景推断: 设计用于手动触发或调试，可能通过 Temporal CLI 手动发送"
...
- `segmentUpdate` 的触发条件在正文和表格中一致（仅进入分段时发送）
